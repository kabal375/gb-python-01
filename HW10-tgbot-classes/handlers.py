...
async def com_help(message: types.Message):
    ...
    await message.answer('/info (после запуска игры) - информация о текущем размере кучи \
        и количестве конфет, которое можно взять за ход')
...
async def com_play(message: types.Message):
    ...
    if game:
        ...
        game = Play.start_game(
            Play, message.from_user.first_name, message.from_id)
